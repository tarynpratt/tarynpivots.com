...
This post has been rattling around in my head for months, and with [SQL Server 2019](https://www.microsoft.com/en-us/sql-server/sql-server-2019) on the horizon, I figured I'd finally put my thoughts down, especially since I know we might hit some of the same issues when we upgrade. 
...
We have various [availability groups](https://docs.microsoft.com/en-us/sql/database-engine/availability-groups/windows/always-on-availability-groups-sql-server?view=sql-server-2017) (AGs) running on the Stack Overflow, Stack Exchange, Internal Development, and Reporting servers. Adding another layer of complexity, we also have availability groups that are a part of [Distributed Availability Groups](https://docs.microsoft.com/en-us/sql/database-engine/availability-groups/windows/distributed-availability-groups?view=sql-server-2017) (DAGs). The main clusters (SO, SE, and Internal) have data that flows from a handful of databases downstream to the reporting servers. It looks a bit like this:
...
I did research, pinged people on [Database Administrators Stack Exchange](https://dba.stackexchange.com/), and poked my co-worker [Nick Craver about it](https://twitter.com/Nick_Craver/status/963443600997736449). From everything I heard and read, the behavior was expected, but incredibly difficult to deal with if you're not upgrading everything at the same time, or in a short amount of time. I wanted to spread the upgrades out over a month to make them manageable, but when they go out of commission because of a upgrade, that timeframe went out the window. 
...
Microsoft recommends [upgrading servers](https://docs.microsoft.com/en-us/sql/database-engine/availability-groups/windows/upgrading-always-on-availability-group-replica-instances?view=sql-server-2017) in this order:
